...
**Head-to-head with large pages** (7 alternating runs, `MIMALLOC_LARGE_OS_PAGES=1`): V8 wins **7/7 runs**. V8 median **8.42s** vs primecount median **8.80s** (−0.38s, **4.34% faster**). Requires `SeLockMemoryPrivilege` granted via `grant_lock_memory.ps1` + reboot.
...
See [OPTIMIZATIONS_V8.md](OPTIMIZATIONS_V8.md) for the full optimization log (155+ experiments).
...
- **Exhaustive table layout analysis** — 155+ experiments tested SegmentedPiTable (segment-first and per-b), mod-240 wheel encoding, interleaved bits+prefix, sparse prefix, wheel-30 BigPiTable, clustered easy leaves, software prefetch, PGO, separate thread pools, LLVM flag tuning, phased scheduling, D segment tuning, P/E-core affinity, rayon chunk sizes, B thread throttling, 1GB huge pages, LTO variants, process priority, CompactPi two-level pi table, split-loop AC, D_THREADS isolation, exhaustive parameter sweeps. All regressed or showed no improvement except CompactPi (-1.2% wall, -150MB memory). The fundamental limit is ROB-limited MLP — any instruction added to the AC inner loop reduces the reorder buffer's look-ahead window and causes regression.
...
- **Head-to-head wins 7/7 vs primecount** — Under identical thermal conditions (alternating runs with large pages), V8 median 8.42s vs primecount median 8.80s. **Beats primecount at Max i64 by 4.34%.**

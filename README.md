...
Exhaustive analysis branch of V7 with **167+ experiments** (Opt 53-156, Sessions 1-14) across table layout, scheduling, prefetching, algorithmic optimizations, compiler tuning, PGO, thread pool strategies, memory access patterns, and OS-level memory management. Deep analysis of primecount's source revealed three key differences: SegmentedPiTable (L1-cached per-segment π lookups), clustered easy leaves, and mod-240 wheel encoding. Sessions 3-4: table layout experiments, phase scheduling, AC_SEG/alpha sweeps, `-Zbuild-std` (only nightly improvement: -1.3%). Sessions 5-6: P-core affinity, per-b SegPiTable, segment-first SegPiTable (5× regression — sieve construction cost dominates), software prefetch (neutral — OoO engine handles it), clustered easy leaves (correct after two bug fixes but 4× slower at scale), PGO+build-std (no improvement), separate thread pools (all worse — oversubscription), LLVM flag tuning (all noise), phased scheduling (AC alone=2.10s confirming 4× concurrent penalty, but sequential phases slower overall), D segment tuning (default optimal). Session 7: DELAY_D scheduling (revealed BigPiTable L3 warming effect — AC keeps 285MB table warm for B), B_THREADS sweep (default B=24 optimal — AC/B balance is zero-sum), POOL_MULT tuning (default ×3 optimal). Session 8: interleaved BigPiTable (5% regression — larger table hurts spatial locality), deep prefetch (4% — competes for L2 miss entries), monotonic sweep with running pi (80% — loss of 4× unrolling MLP is catastrophic), 8× unrolling (4% — register pressure). Session 9: 2MB large pages via `SeLockMemoryPrivilege` + mimalloc (1.3% improvement — 73K→143 TLB entries for 285MB BigPiTable; transparent allocator-level approach beats explicit VirtualAlloc). Session 10: D prefix-sum (catastrophic), B chunks (inconclusive), narrow reorder (noise, kept), two-level prefix (regression), 8× unroll (regression), D Type 2 estimator (neutral, kept), AC prefetch-ahead (regression). Session 11: Wheel-30 BigPiTable (-13%, confirms ROB-MLP principle), interleaved BigPiTable (-2.7%), P/E-core affinity (-75%), rayon min_len (default optimal), B thread throttling (reveals precise bandwidth contention: AC improves 14% at B=4 but B becomes bottleneck), 1GB huge pages (noise), LTO variants (noise), LLVM flags (noise), process priority (noise). Session 12: CompactPi two-level pi table (-150MB memory, -1.2% wall), split-loop AC (-8% I-cache regression), exhaustive parameter sweep (all 8 parameters confirmed at optimum), D_THREADS isolation revealed AC solo=3.1s (2.7× concurrent penalty from L3 contention). Session 13: NTA prefetch (noise), div/mod codegen verified (magic multiply), ValidM struct analysis (16B minimum), alternative allocator (snmalloc CRT mismatch; system allocator 1.6% slower confirming mimalloc+large pages optimal), B delay (0-2s no effect — D is the contention source), segment order (reverse confirmed 1.2% better), hot loop disassembly audit (~92 µops optimal), MSHR saturation analysis (~38/48 DRAM misses, speculative prefetch counterproductive), 8× unroll analysis (MLP invariant to unroll factor). Session 14: **Streaming AC** — implemented per-segment L2-cached sieves to replace 285MB BigPiTable (correct at 10^15 and Max i64, but SLOWER: MLP-38 already provides ~2.6ns effective DRAM latency matching L2, sieve construction overhead 0.7s, load imbalance from segment 0 having 69% of work, nested rayon parallelism degradation). Alpha parameter sweep confirmed current alpha_y=18.5 optimal. **Conclusion: V8 is at the absolute hardware ceiling** — the 2.7× DRAM contention penalty between AC and D is unfixable at the software level on Arrow Lake. **Critical discovery: MSHR-saturated MLP** — with ~38 outstanding DRAM misses and ~48 L2 MSHRs, the system is at 80% MSHR utilization; any additional memory requests reduce effective MLP. **Key V8 optimizations**: Barrett reduction `fast_div` (6× faster than hardware division), branchless `pi_fast` (BMI2 BZHI+POPCNT, 7 instructions), 4× unrolled inner loop with unchecked access (Pareto-optimal MLP vs register pressure), CompactPi two-level pi table (51MB vs 202MB), `build-std` with `-Ztune-cpu=arrowlake`, POOL_MULT=3 global pool with separate B/C1 pools, two-scope early AC start, mimalloc large pages. **Final: 8.38s best, 8.44s median with large pages** (beats primecount by 3.65%). See [OPTIMIZATIONS_V8.md](OPTIMIZATIONS_V8.md) for full experiment log.
...
**Head-to-head with large pages** (5 runs, `MIMALLOC_LARGE_OS_PAGES=1`): V8 wins consistently. V8 median **8.44s** vs primecount median **8.76s** (−0.32s, **3.65% faster**). Requires `SeLockMemoryPrivilege` granted via `grant_lock_memory.ps1` + reboot. 167+ experiments across 14 sessions, including Streaming AC (proven non-viable: MLP-38 already matches L2 effective latency).

**Head-to-head without large pages** (10 alternating runs, identical thermal conditions): V8 wins **9/10 runs**. V8 median 8.63s vs primecount median 8.70s (−0.07s). primecount's published 8.49s is a cold-CPU best; under sustained thermal load it runs 8.70s median, comparable to V8's 8.60s cold-start median.
...
| 100 Trillion | 0.217s | 0.026s | **8.3×** |
| 1 Quadrillion | 0.518s | 0.065s | **8.0×** |
| 10 Quadrillion | 2.319s | 0.222s | **10.4×** |
| 100 Quadrillion | 14.379s | 0.790s | **18.2×** |
| 1 Quintillion | 51.840s | 2.380s | **21.8×** |
| Max i64 | 342.460s | 8.650s | **39.6×** |
...
See [OPTIMIZATIONS_V8.md](OPTIMIZATIONS_V8.md) for the full optimization log (164+ experiments).
...
- **Exhaustive table layout analysis** — 164+ experiments tested SegmentedPiTable (segment-first and per-b), mod-240 wheel encoding, interleaved bits+prefix, sparse prefix, wheel-30 BigPiTable, clustered easy leaves, software prefetch, PGO, separate thread pools, LLVM flag tuning, phased scheduling, D segment tuning, P/E-core affinity, rayon chunk sizes, B thread throttling, 1GB huge pages, LTO variants, process priority, CompactPi two-level pi table, split-loop AC, D_THREADS isolation, exhaustive parameter sweeps, alternative allocators, B delay scheduling, segment ordering, hot loop disassembly auditing, MSHR saturation analysis. All regressed or showed no improvement except CompactPi (-1.2% wall, -150MB memory) and mimalloc+large pages (-1.6%). The fundamental limits are ROB-limited MLP and MSHR saturation — any instruction or memory request added to the AC inner loop reduces effective parallelism.
...
- **Head-to-head wins vs primecount** — Under identical thermal conditions (alternating runs with large pages), V8 median 8.44s vs primecount median 8.76s. **Beats primecount at Max i64 by 3.65%.** 167+ experiments across 14 sessions confirmed V8 is at the absolute hardware ceiling for Gourdon's algorithm on Arrow Lake.

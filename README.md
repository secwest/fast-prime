...
**Head-to-head with large pages** (5 alternating runs, 45s cooldown, `MIMALLOC_LARGE_OS_PAGES=1`): V8 wins **5/5 runs**. V8 median **8.59s** vs primecount median **8.83s** (−0.24s, **2.7% faster**). Requires `SeLockMemoryPrivilege` granted via `grant_lock_memory.ps1` + reboot.
...
See [OPTIMIZATIONS_V8.md](OPTIMIZATIONS_V8.md) for the full optimization log (145+ experiments).
...
- **Exhaustive table layout analysis** — 145+ experiments tested SegmentedPiTable (segment-first and per-b), mod-240 wheel encoding, interleaved bits+prefix, sparse prefix, wheel-30 BigPiTable, clustered easy leaves, software prefetch, PGO, separate thread pools, LLVM flag tuning, phased scheduling, D segment tuning, P/E-core affinity, rayon chunk sizes, B thread throttling, 1GB huge pages, LTO variants, process priority. All regressed or showed no improvement: odd-only 128-per-word with separate prefix/bits arrays and concurrent global pool scheduling is optimal for the b-first architecture. The fundamental limit is ROB-limited MLP — any instruction added to the AC inner loop reduces the reorder buffer's look-ahead window and causes regression.

...
**Head-to-head with large pages** (5 alternating runs, `MIMALLOC_LARGE_OS_PAGES=1`): V8 wins consistently. V8 median **8.44s** vs primecount median **8.76s** (−0.32s, **3.7% faster**). Requires `SeLockMemoryPrivilege` granted via `grant_lock_memory.ps1` + reboot.
...
- **Head-to-head wins vs primecount** — Under identical thermal conditions (alternating runs with large pages), V8 median 8.44s vs primecount median 8.76s. **Beats primecount at Max i64 by 3.7%.**

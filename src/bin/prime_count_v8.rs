...
// ── AC: combined A + C formulas ──────────────────────────────────────────────
// V8: b-first iteration with BigPiTable (proven V7 structure) plus
        ...
        return std::thread::scope(|s| {
            let ac_handle = s.spawn(|| {
                ...
                let r = if let Some(ref pool) = ac_pool_opt {
                    pool.install(|| compute_ac(x, y, z, k, x_star, &primes, &pi, &big_pi, &recip))
                } else {
                    compute_ac(x, y, z, k, x_star, &primes, &pi, &big_pi, &recip)
                };

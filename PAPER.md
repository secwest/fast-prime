...
6. **A case study in human-AI collaborative research**: demonstrating that an LLM-assisted workflow can compress months of optimization work into weeks, while producing documentation (this paper, 1100+ lines of optimization logs, 2700+ lines of thought log) that would typically be omitted from a solo effort. A key methodological finding: **git preserves code; the Copilot Chronicle preserves understanding** -- the persistent SQLite session database introduced mid-project transformed the collaboration from stateless optimization into sustained, systematic research.
...
17. **Git preserves code; the Copilot Chronicle preserves understanding** -- version control records *what* changed across 150+ commits, but the persistent session database records *why*: the disassembly analysis behind the MLP-38 model, the thermal hypothesis that explained run-to-run variance, the five converging lines of evidence for MSHR saturation. Without the reasoning preserved across sessions, the systematic 167+ experiment search would have degenerated into a random walk (§Acknowledgments).
...
Finally, a note on method. This work was produced by a human-AI collaboration in approximately 120 hours of interactive sessions spanning 14 sessions over several weeks. The AI's contribution -- implementing 321+ experiments with full documentation of every success and failure -- would have taken a solo researcher many months. The human's contribution -- the three strategic pivots that provided 99.7% of the speedup -- could not have come from the AI. Critically, this collaboration was shaped by a before-and-after event: the introduction of a persistent SQLite session database to the Copilot CLI, which occurred *during* the project's timeline around the V7/V8 transition. Before the session store, each of the 6+ session crashes required hours of manual context recovery from git history. After it, session interruptions became transparent -- the AI resumed with full knowledge of all prior sessions, including the reasoning behind decisions that git commits alone cannot preserve. This crash resilience was not a luxury; without it, the systematic 167+ experiment search that ultimately proved the hardware floor would have been impractical. The pre-chronicle sessions (V1-V7) were opportunistic and effective at ~77% success rate. The post-chronicle sessions (V8) were systematic and necessarily achieved only 2% -- but it was the systematicness that made the 2% a proof rather than a suspicion. The complete experiment logs, thought journal, and source code are available at https://github.com/secwest/fast-prime.
...
>
> **For anyone working with AI**: The stamina is the AI's gift -- 321+ experiments, each fully documented. The wisdom is the human's -- knowing when to stop optimizing and start over. But neither matters without memory. Git preserves code; the Copilot Chronicle preserves understanding -- the reasoning, the failed hypotheses, the quantitative findings that inform the next experiment. Persistent session infrastructure is the difference between a stateless assistant and a research partner that builds on its own prior work.
...
The collaboration faced a fundamental challenge: context limits. Each interactive session could hold only a fraction of the project's accumulated knowledge (3,900+ lines of thought log, 1,600+ lines of optimization log, 2,500+ lines of implementation code). Power failures, session timeouts, and context window limits interrupted work repeatedly. The project's thought log records at least six session crashes during Sessions 1-10 alone, each requiring painstaking manual recovery from git history and log files. In one case, an entire session's worth of experiments and analysis was lost, requiring the human to reconstruct context by hand -- a process that consumed hours and inevitably lost the reasoning behind decisions, even when the code changes survived in git.

**The introduction of the session database changed everything.** Midway through the project -- around the V7/V8 transition, roughly at the point where the optimization search needed to become systematic rather than opportunistic -- GitHub's Copilot CLI gained a persistent SQLite-backed session store. This was not a feature we requested; it arrived as a platform update during the project's timeline. But its impact was immediate and dramatic.

The session store maintained structured records across sessions: checkpoints with technical summaries, file change history, and FTS5 full-text indexes over all prior conversation. Where a pre-chronicle session crash meant starting from scratch (re-reading the thought log, re-parsing git diffs, re-establishing which experiments had been tried), a post-chronicle session could recover in seconds. The AI could query its own history -- not just the code, but the *reasoning* behind the code: why a particular prefetch distance was chosen, what the disassembly revealed about register pressure, which scheduling variant was tried and why it regressed.

The contrast between the pre-chronicle and post-chronicle eras of this project is quantifiable:

- **Pre-chronicle (V1-V7, ~12 sessions):** ~154 experiments, frequent manual recovery from crashes, experiment strategy limited to what could fit in a single context window. Each session was effectively stateless -- the AI knew only what the human chose to re-inject at session start. Experiments were opportunistic: try what seems promising, move on.
- **Post-chronicle (V8, Sessions 9-14):** 167+ experiments across 6 sessions with zero lost context. Each session began with full knowledge of all prior sessions. The search became *systematic*: maintain a shrinking list of untried strategies, eliminate categories of approaches with evidence, build quantitative models (MLP-38, MSHR saturation) that accumulate data across sessions.

This infrastructure transformed the collaboration in several concrete ways:

1. **Crash resilience.** The 6+ crashes during pre-chronicle sessions each cost hours of recovery time and lost irreproducible reasoning. After the chronicle, session interruptions became painless -- the AI resumed with full context, including mid-experiment state that would have been impossible to reconstruct from git alone. Git preserves *what* changed; the chronicle preserves *why*.

2. **Avoiding redundant work.** When Session 14 set out to implement streaming AC, the AI could query its own history to find that per-segment SegPiTable had already been tried three times (V6, Opts 86-90, Opts 95-98) with known failure modes. This prevented a fourth attempt at the same dead end and instead guided the design toward a fundamentally different streaming approach. Pre-chronicle, the AI had no memory of V6's experiments and would have repeated them.

3. **Building on prior quantitative findings.** The MSHR saturation analysis in Session 13 (Opt 151-152) depended on disassembly data and MLP calculations from Session 11's ROB-limited-MLP discovery. Without persistent session state, these connections would have been lost across context boundaries. The chronicle turned isolated session insights into a cumulative body of evidence.

4. **Enabling systematic exhaustion.** The 167+ V8 experiments were not random explorations but a systematic search guided by a shrinking list of untried strategies, maintained across sessions. Each session could query "what hasn't been tried?" and "what failed for what reason?" to avoid duplication and target the remaining gaps. This level of rigor -- essential for *proving* a hardware floor rather than merely suspecting one -- would have been impractical without persistent state.

5. **Preserving institutional memory.** The success rate collapse data in §3.0 and §6.4 -- covering all 321+ experiments across 8 versions -- could only be computed because the chronicle maintained records from the earliest sessions. A stateless system would have no access to V1-V7 optimization history when analyzing V8 results. The paper itself is an artifact of this persistence: the chronicle made the full optimization arc queryable.

The distinction matters because there is a qualitative difference between "optimization search" and "systematic optimization search." A stateless AI assistant can implement and test individual experiments competently. But the *strategy* -- the decision about which experiment to run *next*, informed by all prior experiments -- requires memory that spans sessions. Pre-chronicle, the human had to serve as that memory, re-injecting prior findings at each session start. This worked for ~50 experiments before the cognitive load became unsustainable. Post-chronicle, the AI could carry its own institutional memory, and the search scaled to 167+ experiments without degradation.

This experience suggests that **persistent session infrastructure is not a convenience but a prerequisite** for AI-assisted research projects that span multiple sessions. The chronicle database enabled a level of systematic rigor that neither a solo human (limited by memory and fatigue) nor a stateless AI (limited by context windows) could achieve alone.
...
- **Session persistence is the scaling bottleneck**: The pre-chronicle/post-chronicle contrast in this project demonstrates that persistent session state is the difference between an AI that can optimize code (any single session) and an AI that can conduct research (sustained multi-session investigation). Without it, V8's systematic exhaustion of the optimization space would have degenerated into a random walk. With it, each session could build on all prior sessions, making the multi-week research arc coherent.
...
- **Git preserves code; the Copilot Chronicle preserves understanding**: Our commit history has 150+ entries, but the commits record what changed, not why. The chronicle's searchable conversation history preserved the reasoning -- the disassembly analysis that led to the MLP-38 model, the thermal hypothesis that explained variance, the five separate lines of evidence that converged on MSHR saturation. This reasoning was essential for designing later experiments and for writing this paper.
